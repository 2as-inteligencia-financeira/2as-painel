    ...
    const fetchData = async () => {
      try {
        ...
        setState(prev => ({
          ...prev,
          loading: false,
          error: error?.message || "Erro ao carregar contas a pagar do Granatum",
        }));
      }
    };
    fetchData();
...
function shapeGranatumContasPagar(payload) {
  const rows = payload?.rows || [];
  const meta = payload?.meta || [];
  return { meta, data: rows };
}

function shapeGranatumComposicoes(payload) {
  const rows = payload?.rows || [];
  const grupos = new Map();

  rows.forEach(row => {
    const naturalKey = chaveComposicao(row.DATA_VENCIMENTO, row.FORNECEDOR, row.DESCRICAO);
    const key = row.ID_COMPOSTO || naturalKey;
    const atual = grupos.get(key) || {
      key,
      dataVencimento: row.DATA_VENCIMENTO,
      fornecedor: row.FORNECEDOR,
      descricao: row.DESCRICAO,
      idComposto: row.ID_COMPOSTO || "",
      bruto: 0,
      deducoes: 0,
      liquido: 0,
      itens: [],
    ...
    atual.bruto += Math.abs(toNum(row.VALOR_BRUTO));
    atual.deducoes += Math.abs(toNum(row.VALOR_DEDUCOES));
    atual.liquido += Math.abs(toNum(row.VALOR_LIQUIDO));
    atual.itens.push({
      tipo: "PROVENTO",
      idPai: row.ID || "",
      descricao: row.DESCRICAO,
      ...
      valor: Math.abs(toNum(row.VALOR_BRUTO)),
    });

    (row.DEDUCOES || []).forEach(item => {
      atual.itens.push({
        tipo: "DEDUCAO",
        idPai: row.ID || "",
        descricao: item.descricao || item.categoria || "Dedução",
        categoriaId: item.categoriaId || item.categoria_id || row.CATEGORIA_ID,
        categoria: item.categoria || row.CATEGORIA,
        valor: Math.abs(toNum(item.valor)),
      });
    });
  ...
  return {
    data: [...grupos.values()].flatMap(grupo =>
      grupo.itens.map(item => ({
        DATA_VENCIMENTO: grupo.dataVencimento,
        FORNECEDOR: grupo.fornecedor,
        DESCRICAO_GRUPO: grupo.descricao,
        DESCRICAO_LANCAMENTO: grupo.descricao,
        ID_COMPOSTO: grupo.idComposto,
        ID_LANCAMENTO_PAI: item.idPai,
        TIPO_ITEM: item.tipo,
        DESCRICAO_ITEM: item.descricao,
        CATEGORIA_ID: item.categoriaId,
        CATEGORIA: item.categoria,
        VALOR_ORIGINAL: item.valor,
        VALOR_ASSINADO: item.tipo === "DEDUCAO" ? -item.valor : item.valor,
        VALOR_BRUTO_GRUPO: grupo.bruto,
        VALOR_DEDUCOES_GRUPO: grupo.deducoes,
        VALOR_LIQUIDO_GRUPO: grupo.liquido,
      }))
    ),
  };
}
...
export default function FluxoProjetadoLabs() {
  const sheets = useSheets(["saldos", "runway", "tabela_auxiliar", "entradas10d", "tabela_resumo"]);
  const granatumContas = useGranatumContasPagarLabs();

  const contasPagarLive = useMemo(() => shapeGranatumContasPagar(granatumContas.data), [granatumContas.data]);
  const composicoesLive = useMemo(() => shapeGranatumComposicoes(granatumContas.data), [granatumContas.data]);
  const sheetsMix = useMemo(() => ({
    ...sheets,
    contas_pagar: {
      ...granatumContas,
      data: contasPagarLive,
      refetch: granatumContas.refresh,
    },
  }), [contasPagarLive, granatumContas, sheets]);
  ...
  const saldoAtualBruto = Array.isArray(sheets.saldos.data) && sheets.saldos.data.length
    ...
    : 0;
  const composicoes = useMemo(() => indexarComposicoes(composicoesLive), [composicoesLive]);
  const contasList  = useMemo(() => extrairContas(contasPagarLive, composicoes), [contasPagarLive, composicoes]);
  const projecaoRaw = extrairProjecao(sheets.tabela_resumo.data, saldoAtualBruto);
  const projecao    = projecaoRaw ?? PROJ_FB;

  const loading     = sheets.saldos.loading || sheets.runway.loading || granatumContas.loading;
  const posicao     = extrairPosicao(sheetsMix, projecao);
  ...
  const lastUpdate = granatumContas.lastUpdate || sheets.saldos.lastUpdate;
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14, gap: 12, flexWrap: "wrap" }}>
          <div>
            ...
            <div style={{ fontSize: 11, color:T.muted, marginTop: 2 }}>
              {granatumContas.loading ? "Carregando..." : `${gruposContasGranatum.length} lançamentos`}
              {agingAtivo !== "todos" && <span style={{ color: T.blue2, marginLeft: 8 }}>• filtro: {agingContas.find(f => f.id === agingAtivo)?.label}</span>}
              {granatumContas.lastUpdate && <span style={{ color: T.grn, marginLeft: 8 }}>• ao vivo</span>}
              {granatumContas.error && <span style={{ color: T.red, marginLeft: 8 }}>• erro: {granatumContas.error}</span>}
            </div>
          </div>
          <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap" }}>
            ...
            <button onClick={granatumContas.refresh} style={{ padding:"6px 10px", borderRadius:6, border:`1px solid ${T.brd}`, background:"transparent", color:T.sub, fontSize:11, cursor:"pointer" }}>
              Atualizar
            </button>
          </div>
        </div>
