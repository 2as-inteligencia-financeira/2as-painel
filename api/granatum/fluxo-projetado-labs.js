...
export default async function handler(req, res) {
  ...
  try {
    ...
    const payload = await connector.getProjectedCashflow({ companyId: req.query?.empresa || "" });
    registerConnectorSync("granatum", "success", { entity: "cashflow", source: payload?.raw?.source || payload?.source || "" });
    return send(res, 200, req.method === "HEAD" ? "" : JSON.stringify(payload), {
      "Content-Type": "application/json; charset=utf-8",
      "Cache-Control": "no-store",
    });
  } catch (error) {
    registerConnectorSync("granatum", "error", { entity: "cashflow", message: error?.message || "Unknown error" });
