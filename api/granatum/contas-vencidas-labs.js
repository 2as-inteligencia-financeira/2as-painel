...
export default async function handler(req, res) {
  ...
  try {
    ...
    const payload = await connector.getOverduePayables({ companyId: req.query?.empresa || "" });
    registerConnectorSync("granatum", "success", { entity: "overdue_payables", source: payload?.raw?.source || payload?.source || "" });
    return send(res, 200, req.method === "HEAD" ? "" : JSON.stringify(payload), {
      "Content-Type": "application/json; charset=utf-8",
      "Cache-Control": "no-store",
    });
  } catch (error) {
    registerConnectorSync("granatum", "error", { entity: "overdue_payables", message: error?.message || "Unknown error" });
